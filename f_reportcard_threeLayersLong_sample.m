function f_reportcard_threeLayersLong_sample(cn,env_n,econ_n,soc_n,dim_n,var_n,dimcolrcode,dimlabels,dimsign,varcolrcode,varlabels)
    ...
    for i = 1:1:dim_n
         colrInd = round(dimcolrcode(cn,i));
         dimcolr{1,i} = [1 0.945 0.749];
    end
    
    varcolr = {};
    for i = 1:1:var_n
        colrInd = varcolrcode(cn,i);
        varcolr{1,i} = [1 0.945 0.749];
    end
 ...
 %{
  ...
  %}
    ...
    text(x+xp(length(ang)-14)-0.15,y+yp(length(ang)-14)-0.15,sprintf('Water \nConsumption'),'FontSize',10,'Rotation', 38,'HorizontalAlignment','center','VerticalAlignment','middle');
    ...
    text(x+xp(length(ang)-18)-0.18,y+yp(length(ang)-18),sprintf('Trade \nOpenness'),'FontSize',10,'Rotation', 0,'HorizontalAlignment','center','VerticalAlignment','middle');
    ...
    text(x+xp(length(ang)-26)-0.05,y+yp(length(ang)-26)+0.2,sprintf('Labor \nProductivity'),'FontSize',10,'Rotation', -80,'HorizontalAlignment','center','VerticalAlignment','middle');

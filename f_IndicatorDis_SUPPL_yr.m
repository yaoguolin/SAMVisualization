function f_IndicatorDis_SUPPL_yr(var,var_comp,var_name,L,U,minV,maxV,minVy,maxVy,ylb, xlb,yr2000_t,dir,FAOSTAT_CoABR_ISO,RegionCoName,RegionID,RegionName)
Yr=1961:2016;
close all;
%yr2000_t= [40 55];
fig_caption={'a','b'};
...
for i=1:length(regions)
    ...
    text(var_comp(idxrgn,yr2000),var(idxrgn,yr2000),char(FAOSTAT_CoABR_ISO(idxrgn)),'Color',colormap(i,:),'Fontweight','bold','FontSize',8);
    hold on
...
ylabel(ylb)
xlabel(xlb);
title(sprintf('(%s) Year %d',char(fig_caption(j)), Yr(yr2000)))
%set(gca,'XtickLabel',[])
set(gca,'Yscale','log')
set(gca,'Xscale','log')
...
print([dir var_name '_SAMSuppInfo_Fig_' num2str(Yr(yr2000_t(1))) 'and' num2str(Yr(yr2000_t(2))) '_FAOabr'],'-dpng','-r200')

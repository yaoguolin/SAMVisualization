...
total_data = load([resultsDir dirSep 'SAMcorr_SSR.mat']);
...
load('C:\Users\gyao\Google Drive\ZhangLabData08222019\SAM\SAM_Model\SAM_Data\SSR\SSR.mat')

Social_S.SSR = ScoreTrans(SSR);
...
for j = 1:1:n
    ...
    k = (i - 1) * n + j;
        if i ~= j           
            ...
            hold on
            s(k) = scatter([1:4],squeeze(tradeoffsyn_frac(i,j,:)),24* ones(4, 1),colorMap,'filled');
            hold on
...
var_name = {"Water withdraw", "N surplus","P surplus","Land use change","Greenhouse gas","Soil erosion",...
    ...
    "Crop diversity","Food affordability","Under-nourishment","Rural poverty","Gender gap","Land right", "Self-sufficiency"}
...
hold off
print([graphsDir dirSep 'Correlation_SAM_03182021_reverse_SSR'], '-djpeg');
